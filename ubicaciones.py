...
def sales_header():
    ...
    st.title('Ubicaciones :earth_americas:')
...
def create_map(the_df):
    ...
    alt_df = alt_df.groupby(["Ubicación", "latitude", "longitude"], as_index=False)[
        "Precio_USD"].sum()

    fig = px.scatter_mapbox(alt_df, lat="latitude",
                            lon="longitude", hover_name="Ubicación",
                                hover_data=["Precio_USD"],
                                color="Precio_USD",
                            size="Precio_USD",
...
def create_subscription_via_location(the_df):
    ...
    fig = px.bar(purchases_by_loc,
         ...
         labels = {"x" : "Total Compras"},
         text_auto="0.4s",  
         title = "Top 10 Número de compras por Estado",
         color = purchases_by_loc.index,
         color_discrete_sequence=["#8DBBDF"],
    ...
    fig.update_layout(
     ...
     xaxis= dict(title='Total Compras',
...
def create_location_category(the_df):
    ...
    fig = px.sunburst(category_via_loc, path=['Ubicación', 'Categoría'],
                      values='count',
                      color_discrete_sequence=[
                          "#B403C2", "#7703C2", "#0329C2", "#03AEC2", "#03C251"],
...
def create_top3_review(the_df):
    ...
    fig = px.bar(loc_review,
                 color_discrete_sequence=["#049FE8"],
                 template="plotly_dark",
                 text_auto="0.1f",
                 labels={
                     "value": "PROM Calficación", "Estado_de_Suscripción": "Estado_de_Suscripción"},
    ...
    fig.update_layout(
        ...
        xaxis= dict(title='Ubicación',
                    title_font=dict(size=18)),
        yaxis=dict(title='PROM Calificación',
                   title_font=dict(size=18)),
    )
